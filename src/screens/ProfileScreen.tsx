...
const ProfileScreen: React.FC = () => {
  const { user, role, loading: authLoading, error: authError, login, logout, clearError } = useAuth();
  ...
  // Sync local error with auth error
  useEffect(() => {
    if (authError) {
  ...
  const handleLogin = async () => {
    ...
    // Clear form after login attempt
    if (!authError) {
      setEmail('');
      setPassword('');
    }
  ...
  const renderLoginForm = () => (
    <View style={styles.formContainer}>
      <View style={styles.iconContainer}>
        <MaterialCommunityIcons name="account-circle" size={80} color="#FF0000" />
      </View>
      ...
      {localError ? (
        <View style={styles.errorContainer}>
          <MaterialCommunityIcons name="alert-circle" size={18} color="#FF0000" />
  ...
  const renderLoggedInState = () => (
    <View style={styles.profileContainer}>
      <View style={styles.avatarContainer}>
        <MaterialCommunityIcons name="account-circle" size={100} color="#FF0000" />
      </View>
      ...
      <View style={styles.userInfoContainer}>
        <MaterialCommunityIcons name="shield-account" size={24} color="#FF0000" />
        <View style={styles.userInfoTextContainer}>
          <Text style={styles.userInfoLabel}>Rol</Text>
          <Text style={styles.userInfoValue}>
            {role === 'kitchen' ? 'Kitchen' : role === 'courier' ? 'Courier' : 'User'}
          </Text>
        </View>
      </View>
      ...
      {/* Telegram Link for regular users */}
      {role === 'user' && (
        <TouchableOpacity
